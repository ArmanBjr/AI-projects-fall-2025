...
class Problem:
    ...
    def check_down(block: Block, board: Board) -> bool:
        if not block.is_vertical or block.position[block.last_position - 1][0] == board.height - 1 or board.gird[block.position[block.last_position - 1][0] + 1][
            block.position[block.last_position - 1][1]] != 0:
            return False
        return True
    ...
    def check_right(block: Block, board: Board) -> bool:
        if block.is_vertical or block.position[block.last_position - 1][1] == board.width - 1 or board.gird[block.position[block.last_position - 1][0]][
            block.position[block.last_position - 1][1] + 1] != 0:
            return False
        return True
