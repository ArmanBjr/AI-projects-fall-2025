    ...
    def extract_solution(self, goals, level_index):
        ...
        for action_set in possible_action_sets:
            ...
            for action in action_set:
                for p in action.positive_preconditions:
                    new_goals.add((p, True))
                for p in action.negative_preconditions:
                    new_goals.add((p, False))

            
            # che pish shart hayyee inja boodan baraye ma
            
            # find plan_prefix
            plan_prefix = self.extract_solution(new_goals, level_index - 2)

            # khodesh ro bazgashti seda bezanim roo be aghab
        ...
        for r in range(1, len(relevant_actions) + 1):
            for action_set in combinations(relevant_actions, r):
                ...
                for a1, a2 in combinations(action_set, 2):
                    if frozenset({a1, a2}) in action_layer.action_mutex:
                        mutex_found = True
                        # action_set.remove(a1)
                        # action_set.remove(a2)
                        break

                # Step 5: If mutex-free, add to results
                if not(mutex_found):
                    results.append(list(action_set))
