    ...
    def _calculate_action_mutexes(self, action_layer, last_state):
        ...
        for i in range(len(actions)):
            for j in range(i + 1, len(actions)):
                ...
                # result1 = collections.Counter(a1.add_list) & collections.Counter(a2.delete_list)
                # result2 = collections.Counter(a1.add_list) & collections.Counter(a2.delete_list)
                result11 = not(a1.add_list.isdisjoint(a2.delete_list))
                result12 = not(a1.delete_list.isdisjoint(a2.add_list))
                # Reason 1: Inconsistent Effects
                if result11 or result12:
                    mutex = True

                
                # Reason 2: Interference
                result21 = not(a1.positive_preconditions.isdisjoint(a2.delete_list))
                result22 = not(a2.positive_preconditions.isdisjoint(a1.delete_list))
                result23 = not(a2.negative_preconditions.isdisjoint(a1.add_list))
                result24 = not(a1.negative_preconditions.isdisjoint(a2.add_list))
                if result21 or result22 or result23 or result24:
                    mutex = True
                ...
                if not mutex:
                    ...
                    for lit1 in pre1:
                        for lit2 in pre2:
                            if frozenset({lit1, lit2}) in last_state.literal_mutex:
                                mutex = True
                                break
